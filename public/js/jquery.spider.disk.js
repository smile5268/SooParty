...
(function ($){
	var defaults = {
		...
		itemStyle:{
			...
			borderColor:'#e5e5e5',		//边框线颜色
			normalBgColor:'#0066cc',	//正常 百分比颜色
			waringBgColor:'#F55656',		//超过预警值 百分比颜色
		...
		$.each(dataObj.disks,function (i,n){
			...
			if(imgWidth>0)
			{
				if(i>(options.itelTotal-1))
					...
					index = i;
					itemDiv="<div style='border:1px solid "+ options.itemStyle.borderColor+";background-color:"+options.itemStyle.bgColor+";font-size:"+options.itemStyle.fontSize+";overflow: hidden;"+"border-radius: 20px;"+"'>";
			...
			//"<tr></tr>"
			var tds="<td width='30%' style='bgcolor: "+options.itemStyle.bgColor+";'>"+itemDiv+"</td>";
			if(i%2==0){
				trs+="<tr>"+tds;
			}else{
				trs+=tds+"</tr>";
