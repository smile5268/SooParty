...
(function() {
    ...
    anjiehotel.module.address = function(k) {
        ...
        k.$r("onblur",function() {
            ...
            var clears = $(this).val(); // 筛选的地址
            var sties = window.location.href;
            var mms = decodeURI(sties);     // 汉字解密
            var addressReg = /[&][a][d][d][r][e][s][s]=[^\x00-\xff]{1,6}/;    //地点正则
            var addressLike = mms.replace(addressReg,"");   // address如果有筛选掉
            var pageAdd = /[p][a][g][e]=[0-9]{1,4}/;        // page正则
            var newUrlAdd = addressLike.replace(pageAdd,"");    // 去掉page
            var logAdd = newUrlAdd+'&address='+clears;      // 生成新的链接
            location.href = logAdd;         // 转跳页面
